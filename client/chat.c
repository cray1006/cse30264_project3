...
struct client{	//client structure that holds an address structure and a username
...
int main(int argc, char *argv[]){
	...
	struct client client_list[MAX_CLIENTS]; //array of client structures that holds which clients are in our group
	srand(time(NULL));			
	struct timeval myTime;			//used with select function
	myTime.tv_sec = 300;			//setting timeout for 6 minutes
	myTime.tv_usec = 0;
	fd_set fds;				//used with select function
	int i, j, k;
	for(i = 0; i < MAX_CLIENTS; i++){	//initializing the client list
	...
	clientAddr.sin_family = AF_INET;	//filling clientAddr struct
	...
	if(argc <= 1)
	{
		perror("not enough arguments\n"); //kill program if user did not input server
		return 0;
	}
	else
	{
		//parsing through the user's argument to get the server name and address as well as the desired port number
		...
		if(port != DPORT)
		{
			port = DPORT; //while a user can specify a port, for the sake of testing/grading port stuck at default 9421
		}
	}
	...
	while(1)
	{
		...
		p2p = select(maxfd + 1, &fds, NULL, NULL, &myTime); //using select to multiplex input from the socket and stdin
		if(p2p == -1)
		{
			perror("select");
			return 0;
		...
		{
			printf("Timeout, program ending.\n"); //program automatically ends after 5 minutes of inactivity
			return 0;
		}

		if(FD_ISSET(sockfd, &fds)) //activity at the socket detected
		{
			//receiving input from either server or another client
			recvlen = recvfrom(sockfd, tempBuffer1, BUFSIZE, 0, (struct sockaddr *)&(clientAddr), &len);
			if(recvlen > 0)
			{
				tempBuffer1[recvlen] = 0;
				if(tempBuffer1[0] == 'D') //message from server indicating someone left the group
				{
					i = 0;
				...
				else if(tempBuffer1[0] == 'T') //p2p message received
				{
					i = 0;
					for(j = 2;tempBuffer1[j] != ':'; j++, i++)
					{
					...
					if((strcmp(user, lastUser) != 0) || (ID != last_ID)) //checking to make sure that this message is not a duplicate
					{
						...
						Mix_PlayChannel( -1, ding, 0 );		//playing ding sound effect
						printf("%s > %s\n", user, tempBuffer2);	//printing message and its sender

						for(i = 0; i < MAX_CLIENTS; i++) //forwarding message to other clients
						{
							if(strcmp(client_list[i].username, "EMPTY") != 0)
							{
						...
						for(i = 0; i < MAX_CLIENTS; i++) //adding client to our client list if they do not have an entry
						{
		...
		if(FD_ISSET(fileno(stdin), &fds)) //received input from stdin
		{
			fgets (input, 64, stdin);
			if(strcmp(input, "list\n") == 0) //request list of groups from server
			{
				strcpy(sendBuffer,"L:");
				#ifdef DEBUG
			...
			else if((strcmp(input, "join\n") == 0) && (strcmp(header, p2pHeader) == 0)) //request to join a group
			{
				for(i = 0; i < MAX_CLIENTS; i++) //clear client list
				{
					strcpy(client_list[i].username, "EMPTY");
				}
			...
			else if((strcmp(input, "leave\n") == 0) && (strcmp(header, p2pHeader) != 0)) //request to leave the group
			{
				sendto(sockfd, "D::", 3, 0, (struct sockaddr *)&serverAddr, sizeof(serverAddr)); //send request to server
				strcpy(header, p2pHeader);
				strcpy(myGroup, "there is no way out");
			}
			else if(strcmp(input, "quit\n") == 0) //request to quit the program
			{
				if(strcmp(header, p2pHeader) != 0) //leave the current group if you are in one
				{
					sendto(sockfd, "D::", 3, 0, (struct sockaddr *)&serverAddr, sizeof(serverAddr));
				}
				Mix_FreeChunk(ding); //freeing the SDL resources
				ding = NULL;
				Mix_Quit();
				SDL_Quit();
			...
			else if((strcmp(input, "send\n") == 0) && (strcmp(header, p2pHeader) != 0)) //sending a message to other clients in group
			{
				fgets (tempBuffer2, BUFSIZE, stdin); //formatting message
				strcpy(sendBuffer, "T:");
				...
				for(i = 0; i < MAX_CLIENTS; i++) //sending message to all group members
				{
					if(strcmp(client_list[i].username, "EMPTY") != 0)
					{
			...
			else if((strcmp(input, "help\n") == 0)) //displaying all available commands
			{	
			...
			else if((strcmp(input, "glist\n") == 0) && (strcmp(header, p2pHeader) != 0)) //displaying group members
			{
				for(i = 0; i < MAX_CLIENTS; i++)
				{
					if(strcmp(client_list[i].username, "EMPTY") != 0)
